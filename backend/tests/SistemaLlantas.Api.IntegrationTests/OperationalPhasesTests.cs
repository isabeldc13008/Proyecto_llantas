...
public sealed class OperationalPhasesTests:IClassFixture<WebApplicationFactory<Program>>
{
 ...
 public async Task ServicioLlanta_ExigeAprobacionAntesDeEnvio()
 {
  ...
  {
   await using var tx=await db.Database.BeginTransactionAsync();var tire=await db.Llantas.AsNoTracking().FirstAsync(x=>!db.OrdenesServicioLlanta.Any(o=>o.LlantaId==x.Id&&o.Activo));var controller=new ServiciosLlantaController(db,scope.ServiceProvider.GetRequiredService<IOperacionService>(),scope.ServiceProvider.GetRequiredService<ICicloVidaLlantaService>(),scope.ServiceProvider.GetRequiredService<IWebHostEnvironment>()){ControllerContext=Context("qa-servicios","centros.ver_todos","servicios_llanta.consultar","servicios_llanta.gestionar","servicios_llanta.aprobar_propia")};
   var created=await controller.Crear(new("Reparacion",tire.Id,null,null,"Diagnóstico QA",null),CancellationToken.None);var pending=Assert.IsType<ServiciosLlantaController.OrdenDto>(Assert.IsType<CreatedResult>(created.Result).Value);Assert.Equal("OPCIONADA",pending.Estado);await Assert.ThrowsAsync<SistemaLlantas.Application.Common.ConflictoException>(()=>controller.Enviar(pending.Id,CancellationToken.None));var approved=await controller.Aprobar(pending.Id,CancellationToken.None);Assert.Equal("APROBADA",approved.Estado);Assert.Equal("qa-servicios",(await db.OrdenesServicioLlanta.SingleAsync(x=>x.Id==pending.Id)).Aprobador);await tx.RollbackAsync();
  });
 }
