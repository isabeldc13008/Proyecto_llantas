...
{
 [Theory][InlineData("success",100000)][InlineData("success",105000)][InlineData("rollback",105000)][InlineData("odometer",99999)][InlineData("duplicate",100000)][InlineData("cancel",100000)]
 public async Task AsignacionReservaEjecucionAtomica(string scenario,decimal kilometraje)
 {
  ...
  for(var i=0;i<2;i++)await operations.MoverAsync(new(){LlantaId=tires[i].Id,PosicionDestinoId=positions[i].Id,TipoDestino="Posicion",Motivo="Preparar prueba",KilometrajeVehiculo=100000},tech.Username,access,ct);
  ...
  var work=await controller.TrabajoMontaje(plan.Id,ct);Assert.Contains(work.Asignaciones,x=>x.LlantaId==tires[2].Id);Assert.Equal(2,work.Asignaciones.Count);
  if(scenario is "rollback" or "odometer")
  {
   if(scenario=="rollback"){var oldRows=await db.AsignacionesLlantaPosicion.Where(a=>a.EsActiva&&(a.LlantaId==tires[0].Id||a.LlantaId==tires[1].Id)).ToListAsync();foreach(var a in oldRows)a.KilometrajeMontaje=a.LlantaId==tires[0].Id?90000:110000;await db.SaveChangesAsync();}
   var error=await Assert.ThrowsAsync<ValidacionException>(()=>controller.EjecutarTrabajo(plan.Id,new(kilometraje),ct));if(scenario=="rollback")Assert.Contains("P2",error.Message);Assert.DoesNotContain(db.ChangeTracker.Entries(),e=>e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted);db.ChangeTracker.Clear();Assert.Equal(100000,(await db.Vehiculos.SingleAsync(v=>v.Id==vehicle.Id)).Kilometraje);
  ...
  await controller.EjecutarTrabajo(plan.Id,new(kilometraje),ct);db.ChangeTracker.Clear();
  Assert.Equal(kilometraje,(await db.Vehiculos.SingleAsync(v=>v.Id==vehicle.Id)).Kilometraje);
  var allTires=await db.Llantas.Include(t=>t.EstadoLlanta).Where(t=>tires.Select(x=>x.Id).Contains(t.Id)).ToListAsync();foreach(var t in allTires){if(t.Id==tires[0].Id||t.Id==tires[1].Id){Assert.Contains(t.EstadoLlanta.Codigo,new[]{"DISPONIBLE","DIS"});Assert.Equal("Inventario",t.UbicacionActual);}else Assert.Contains(t.EstadoLlanta.Codigo,new[]{"MONTADA","MON"});}
  Assert.False(await db.AsignacionesLlantaPosicion.AnyAsync(a=>a.EsActiva&&(a.LlantaId==tires[0].Id||a.LlantaId==tires[1].Id)));
  ...
  Assert.Equal(2,await db.AsignacionesLlantaPosicion.CountAsync(a=>a.EsActiva&&(a.LlantaId==tires[2].Id||a.LlantaId==tires[3].Id)));await Assert.ThrowsAsync<ConflictoException>(()=>controller.EjecutarTrabajo(plan.Id,new(kilometraje),ct));
 }
}
