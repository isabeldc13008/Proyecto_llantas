...
public sealed partial class ProgramacionService(LlantasDbContext db):IProgramacionService
{
    public async Task<IReadOnlyList<ProgramacionDto>> ListarAsync(ProgramacionFiltro f,AlcanceCentros alcance,CancellationToken ct)
    {
        ...
        if(!string.IsNullOrWhiteSpace(f.Estado))
        {
            ...
            else if(f.Estado.Equals("Programada",StringComparison.OrdinalIgnoreCase))q=q.Where(x=>x.Estado==EstadoActividad.Pendiente&&x.FechaProgramada>=DateTimeOffset.UtcNow);
            else if(f.Estado.Equals("Finalizada",StringComparison.OrdinalIgnoreCase))q=q.Where(x=>x.Estado==EstadoActividad.Cumplida);
    ...
    {
        if(!string.IsNullOrWhiteSpace(dto.IdempotencyKey)){var existing=await Base(alcance).SingleOrDefaultAsync(x=>x.IdempotencyKey==dto.IdempotencyKey,ct);if(existing is not null)return Map(existing);}
        return await Transaccion(async()=>{
        await Validar(dto,alcance,null,ct);var item=await CrearTrabajo(dto,usuario,alcance,ct);return await Obtener(item.Id,alcance,ct);
        },ct);
    }
    ...
    public async Task<IReadOnlyList<ProgramacionDto>> CrearMasivaAsync(ProgramacionMasivaDto dto,string usuario,AlcanceCentros alcance,CancellationToken ct)
    {
        ...
        return await Transaccion<IReadOnlyList<ProgramacionDto>>(async()=>{var ids=new List<Guid>();
        foreach(var input in dto.Actividades){await Validar(input,alcance,null,ct);var item=await CrearTrabajo(input,usuario,alcance,ct);ids.Add(item.Id);}
        var rows=await Base(alcance).Where(x=>ids.Contains(x.Id)).OrderBy(x=>x.FechaProgramada).ToListAsync(ct);return rows.Select(Map).ToList();},ct);
    }
    ...
    public async Task<ProgramacionDto> ActualizarAsync(Guid id,GuardarProgramacionDto dto,string usuario,AlcanceCentros alcance,CancellationToken ct)
    {
        ...
        if(EsMontaje(item.TipoActividad)||EsMontaje(dto.Tipo))throw new ConflictoException("Cancela el trabajo completo y prográmalo de nuevo para cambiar una asignación de montaje.");
    ...
    {
        if(string.IsNullOrWhiteSpace(dto.Motivo))throw new ValidacionException("El motivo de cancelación es obligatorio.");
        return await Transaccion(async()=>{await CancelarTrabajo(id,dto.Motivo,usuario,alcance,ct);return await Obtener(id,alcance,ct);},ct);
    }

    public async Task EliminarAsync(Guid id,string usuario,AlcanceCentros alcance,CancellationToken ct)
    {
        var item=await db.ActividadesProgramadas.SingleOrDefaultAsync(x=>x.Id==id&&(alcance.VerTodos||alcance.CentroIds.Contains(x.CentroId)),ct)??throw new KeyNotFoundException("Programación no encontrada.");if(EsMontaje(item.TipoActividad)){await CancelarAsync(id,new("Programación eliminada por el administrador"),usuario,alcance,ct);return;}if(item.Estado!=EstadoActividad.Pendiente||item.FechaInicioReal.HasValue)throw new ConflictoException("Sólo puede eliminarse una actividad pendiente que no haya iniciado.");item.Activo=false;item.UsuarioModificacion=usuario;item.FechaModificacion=DateTimeOffset.UtcNow;await db.SaveChangesAsync(ct);
    }
    ...
    private async Task<ActividadProgramada> Construir(GuardarProgramacionDto dto,string usuario,CancellationToken ct){var tech=await db.UsuariosSistema.SingleAsync(x=>x.Id==dto.TecnicoUsuarioId,ct);return new(){TipoActividad=dto.Tipo.Trim(),FechaProgramada=dto.Inicio,FechaFinProgramada=dto.Fin,CentroId=dto.CentroId,VehiculoId=dto.VehiculoId,TecnicoUsuarioId=tech.Id,TecnicoId=tech.Username,Prioridad=dto.Prioridad,Observaciones=dto.Observaciones,Origen=NormalizarOrigen(dto.Origen),OrigenEntidadId=dto.OrigenEntidadId,IdempotencyKey=dto.IdempotencyKey,UsuarioCreacion=usuario};}
    private ProgramacionDto Map(ActividadProgramada x){var end=x.FechaFinProgramada??x.FechaProgramada;var overlap=db.ActividadesProgramadas.AsNoTracking().Any(a=>a.Activo&&a.Id!=x.Id&&(!x.GrupoProgramacionId.HasValue||a.GrupoProgramacionId!=x.GrupoProgramacionId)&&a.TecnicoUsuarioId==x.TecnicoUsuarioId&&a.Estado!=EstadoActividad.Cancelada&&a.FechaProgramada<end&&(a.FechaFinProgramada??a.FechaProgramada)>x.FechaProgramada);var state=x.Estado switch{EstadoActividad.Pendiente when x.FechaProgramada<DateTimeOffset.UtcNow=>"Vencida",EstadoActividad.Pendiente=>"Programada",EstadoActividad.Cumplida=>"Finalizada",EstadoActividad.EnEjecucion=>"EnEjecucion",_=>x.Estado.ToString()};return new(x.Id,x.TipoActividad,x.FechaProgramada,x.FechaFinProgramada,x.CentroId,x.Centro.Nombre,x.VehiculoId,x.Vehiculo is null?"Sin vehículo":$"Interno {x.Vehiculo.NumeroInterno} - {x.Vehiculo.Placa}",x.TecnicoUsuarioId,x.TecnicoId,x.TecnicoUsuario?.Nombre??x.TecnicoId,x.Prioridad,state,Ruta(x),overlap,x.Observaciones,x.MotivoCancelacion,x.FechaFinReal,x.Origen,x.OrigenEntidadId,x.RowVersion,x.GrupoProgramacionId,x.LlantaId,x.PosicionVehiculoId);}
    private static string NormalizarOrigen(string? value){var origin=(value??"MANUAL").Trim().ToUpperInvariant();return new[]{"INSPECCION","ALERTA","INCONSISTENCIA","SOLICITUD","PREVENTIVO","MANUAL"}.Contains(origin)?origin:throw new ValidacionException("Origen de programación inválido.");}
