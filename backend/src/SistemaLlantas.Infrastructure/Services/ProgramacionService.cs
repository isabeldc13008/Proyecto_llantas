...
public sealed class ProgramacionService(LlantasDbContext db):IProgramacionService
{
    ...
    private ProgramacionDto Map(ActividadProgramada x){var end=x.FechaFinProgramada??x.FechaProgramada;var overlap=db.ActividadesProgramadas.AsNoTracking().Any(a=>a.Activo&&a.Id!=x.Id&&a.TecnicoUsuarioId==x.TecnicoUsuarioId&&a.Estado!=EstadoActividad.Cancelada&&a.FechaProgramada<end&&(a.FechaFinProgramada??a.FechaProgramada)>x.FechaProgramada);var state=x.Estado switch{EstadoActividad.Pendiente when x.FechaProgramada<DateTimeOffset.UtcNow=>"Vencida",EstadoActividad.Pendiente=>"Programada",EstadoActividad.Cumplida=>"Finalizada",EstadoActividad.EnEjecucion=>"EnEjecucion",_=>x.Estado.ToString()};return new(x.Id,x.TipoActividad,x.FechaProgramada,x.FechaFinProgramada,x.CentroId,x.Centro.Nombre,x.VehiculoId,x.Vehiculo is null?"Sin vehículo":$"Interno {x.Vehiculo.NumeroInterno} - {x.Vehiculo.Placa}",x.TecnicoUsuarioId,x.TecnicoId,x.TecnicoUsuario?.Nombre??x.TecnicoId,x.Prioridad,state,Ruta(x),overlap,x.Observaciones,x.MotivoCancelacion,x.FechaFinReal,x.Origen,x.OrigenEntidadId,x.RowVersion);}
