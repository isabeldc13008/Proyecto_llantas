...
public sealed class InspeccionService(LlantasDbContext db) : IInspeccionService
{
    ...
    {
        var q = db.Vehiculos.AsNoTracking().Where(x => x.Activo && (alcance.VerTodos || alcance.CentroIds.Contains(x.CentroId)));
        if (soloAsignados)
    ...
    public async Task<InspeccionDto> CrearAsync(CrearInspeccionDto dto, string usuario, AlcanceCentros alcance, CancellationToken ct, bool permitirVehiculoGlobal = false)
    {
        ...
        var vehiculo = await db.Vehiculos.Include(x => x.Ejes).ThenInclude(x => x.Posiciones)
            .SingleOrDefaultAsync(x => x.Id == dto.VehiculoId && x.Activo && (alcance.VerTodos || alcance.CentroIds.Contains(x.CentroId)), ct)
    ...
    {
        var item = await ConsultaCompleta().AsNoTracking().SingleOrDefaultAsync(x => x.Id == id && (alcance.VerTodos || alcance.CentroIds.Contains(x.Vehiculo.CentroId)) && (!permitirPropia||x.TecnicoId==usuario), ct);
