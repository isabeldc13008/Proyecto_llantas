...
public sealed class CicloVidaLlantaService(LlantasDbContext db,ILlantaService llantas):ICicloVidaLlantaService
{
 public async Task<LlantaDetalleDto?> ObtenerDetalleAsync(Guid id,AlcanceCentros alcance,CancellationToken ct)
 {
  ...
  var assignments=await db.AsignacionesLlantaPosicion.AsNoTracking().Where(x=>x.LlantaId==id).Include(x=>x.PosicionVehiculo).ThenInclude(x=>x.EjeVehiculo).ThenInclude(x=>x.Vehiculo).OrderBy(x=>x.FechaInicio).ToListAsync(ct);
  ...
  var mounts=assignments.OrderByDescending(x=>x.FechaInicio).Select(a=>new MontajeVidaDto(a.FechaInicio,a.FechaFin,a.PosicionVehiculo.EjeVehiculo.Vehiculo.NumeroInterno,a.PosicionVehiculo.EjeVehiculo.Vehiculo.Placa,a.PosicionVehiculo.Codigo,a.KilometrajeMontaje,a.KilometrajeDesmontaje,a.KilometrajeRecorrido,a.EsActiva)).ToList();
