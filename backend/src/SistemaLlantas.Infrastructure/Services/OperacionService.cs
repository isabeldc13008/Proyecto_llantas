...
public sealed class OperacionService(LlantasDbContext db) : IOperacionService
{
    ...
    private async Task<MovimientoDto> MoverCoreAsync(EjecutarMovimientoDto dto,string usuario,AlcanceCentros alcance,CancellationToken ct)
    {
        ...
        var mov=new Movimiento{Numero=$"MOV-{DateTimeOffset.UtcNow:yyyyMMddHHmmssfff}-{Guid.NewGuid():N}"[..28],Tipo="Movimiento",Motivo=dto.Motivo,Observaciones=dto.Observaciones,CentroId=llanta.CentroId,Usuario=usuario,UsuarioCreacion=usuario};
