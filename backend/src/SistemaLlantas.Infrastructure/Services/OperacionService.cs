...
public sealed class OperacionService(LlantasDbContext db) : IOperacionService
{
    ...
    public Task ValidarMontajeAsync(Guid llantaId, Guid posicionId, decimal? kilometraje, AlcanceCentros alcance, CancellationToken ct) => ValidarMontajeCoreAsync(llantaId,posicionId,kilometraje,alcance,ct,false);
    ...
    private async Task ValidarMontajeCoreAsync(Guid llantaId, Guid posicionId, decimal? kilometraje, AlcanceCentros alcance, CancellationToken ct,bool permitirCentroDistinto)
    {
        ...
        var tire = await db.Llantas.Include(x=>x.EstadoLlanta).SingleOrDefaultAsync(x=>x.Id==llantaId && x.Activo && x.Centro.Activo && (alcance.VerTodos || alcance.CentroIds.Contains(x.CentroId)),ct)
            ?? throw new KeyNotFoundException("Llanta no disponible en tus centros.");
        var position = await db.PosicionesVehiculo.Include(x=>x.EjeVehiculo).ThenInclude(x=>x.Vehiculo).SingleOrDefaultAsync(x=>x.Id==posicionId && x.Activo && x.EjeVehiculo.Activo && x.EjeVehiculo.Vehiculo.Activo && x.EjeVehiculo.Vehiculo.Centro.Activo && (alcance.VerTodos || alcance.CentroIds.Contains(x.EjeVehiculo.Vehiculo.CentroId)),ct)
            ?? throw new UnauthorizedAccessException("Posición o vehículo no autorizado.");
        ...
        if (!permitirCentroDistinto && tire.CentroId != position.EjeVehiculo.Vehiculo.CentroId) throw new ValidacionException("La llanta debe estar recibida en el centro del vehículo.");
    ...
    private async Task<MovimientoDto> MoverCoreAsync(EjecutarMovimientoDto dto,string usuario,AlcanceCentros alcance,CancellationToken ct,bool inspeccion=false)
    {
        ...
        if(dto.PosicionDestinoId.HasValue && !dto.PosicionOrigenId.HasValue) await ValidarMontajeCoreAsync(dto.LlantaId,dto.PosicionDestinoId.Value,dto.KilometrajeVehiculo,alcance,ct,inspeccion);
