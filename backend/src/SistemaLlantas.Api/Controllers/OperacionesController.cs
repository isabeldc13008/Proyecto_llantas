...
public sealed class OperacionesController(IOperacionService service,ICicloVidaLlantaService ciclo,LlantasDbContext db):ControllerBase
{
    ...
    public async Task<IReadOnlyList<MovimientoTrazabilidadDto>> Movimientos(CancellationToken ct)
    {
        var a=User.AlcanceCentros();
        return await db.MovimientosDetalle.AsNoTracking().Where(x=>x.Activo&&(a.VerTodos||a.CentroIds.Contains(x.Movimiento.CentroId))).OrderByDescending(x=>x.Movimiento.FechaCreacion).Take(500)
