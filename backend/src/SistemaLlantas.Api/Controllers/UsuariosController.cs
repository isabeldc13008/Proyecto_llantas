...
public sealed class UsuariosController(LlantasDbContext db,IConfiguration config):ControllerBase
{
 ...
 internal static UsuarioSistema CrearEntidad(GuardarUsuario dto,string autor)
 {
  if(string.IsNullOrWhiteSpace(dto.Password))throw new ArgumentException("Contraseña requerida.");
  var user=new UsuarioSistema{Username=dto.Username.Trim().ToLowerInvariant(),Nombre=dto.Nombre.Trim(),RolId=dto.RolId,UsuarioCreacion=autor};
  user.PasswordHash=new PasswordHasher<UsuarioSistema>().HashPassword(user,dto.Password);
  foreach(var centroId in dto.CentroIds.Distinct())user.Centros.Add(new(){CentroId=centroId,UsuarioCreacion=autor});
  return user;
 }
