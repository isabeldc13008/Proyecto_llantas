...
export class TiresApi {
  ...
  private withFilters(params:HttpParams,filters:Record<string,unknown>){const centers=filters['centroIds'] as string[]|undefined;const states=filters['estados'] as string[]|undefined;if(centers?.length)params=params.set('centroIds',centers.join(','));if(states?.length)params=params.set('estados',states.join(','));for(const key of ['profundidadMin','profundidadMax','marcaId','referenciaId','dimensionId','tipoLlantaId','tieneReencauches','reencauchesMin','tieneReparaciones','vehiculo','kilometrajeMin','kilometrajeMax','inspeccionDesde','inspeccionHasta','requiereAtencion','activo'])if(filters[key]!==''&&filters[key]!=null)params=params.set(key,String(filters[key]));return params}
