...
export class TireLifecycleDrawer{@Input({required:true})detail!:TireDetail;@Input()centers:CatalogItem[]=[];@Input()canTransfer=false;@Output()closed=new EventEmitter<void>();@Output()transferred=new EventEmitter<{centerId:string;reason:string;notes:string}>();tab:'resumen'|'montajes'|'inspecciones'|'servicios'|'movimientos'|'historial'='resumen';centerId='';reason='';notes='';setTab(value:string){this.tab=value as typeof this.tab}submit(){if(this.centerId&&this.reason)this.transferred.emit({centerId:this.centerId,reason:this.reason,notes:this.notes})}}
