...
<article class="panel form"><label>Operación<select [(ngModel)]="type" (change)="resetSelection()"><option value="Montaje">Montaje</option><option value="Reemplazar llanta">Reemplazar llanta</option><option value="Desmontaje">Desmontaje</option><option value="Rotación">Rotación / cambio de posición</option><option value="Traslado">Traslado entre centros</option><option value="Inventario">Enviar a inventario</option><option value="Reparación">Enviar a reparación</option><option value="Reencauche">Enviar a reencauche</option></select></label>
@if(type==='Montaje'||type==='Reemplazar llanta'){@if(type==='Reemplazar llanta'){<p class="selection">Llanta actual: {{selectedPosition()?.tire??'Selecciona una posición ocupada'}}</p>}<label>Buscar llanta por código o serial<input [(ngModel)]="tireSearch" (keyup.enter)="loadAvailable()"><button (click)="loadAvailable()">Buscar</button></label><small>Hasta 50 disponibles del centro del vehículo. Busca por código o serial para precisar.</small><label>Llanta disponible<select [(ngModel)]="tireId"><option value="">Seleccionar</option>@for(t of available();track t.id){<option [value]="t.id">{{t.codigo}} · {{t.serial}} · {{t.dimension}} · {{t.estado}}</option>}</select></label><p class="selection">Destino: {{selectedPosition()?.code??(type==='Reemplazar llanta'?'Selecciona una posición ocupada':'Selecciona una posición libre')}}</p>}@else{<p class="selection">Origen: {{selectedPosition()?.code??'Selecciona una posición ocupada'}} · {{selectedPosition()?.tire??'—'}}</p>}
@if(type==='Rotación'){<label>Posición destino<select [(ngModel)]="destinationPositionId"><option value="">Seleccionar</option>@for(p of positions();track p.id){<option [value]="p.id">{{p.codigo}} · {{p.llantaCodigo??'Libre'}}</option>}</select></label>@if(destinationOccupant()){<div class="warning">La posición está ocupada por {{destinationOccupant()!.llantaCodigo}}.<span>Se intercambiará con la posición de origen, dentro del mismo vehículo.</span></div>}}
...
</main>`,styles:[`.ops{display:grid;gap:1rem;box-sizing:border-box;width:100%;max-width:none;padding:clamp(1rem,2.4vw,2rem)}.ops>*{min-width:0;max-width:100%}.grid>*{min-width:0}.ops>nav{display:flex;gap:.5rem;flex-wrap:wrap}.ops>nav button{border:1px solid #d3e3d4;background:#f2f8ee;padding:.7rem;border-radius:9px}.ops>nav button.primary{width:auto;background:#315f22}.ops>header p{color:var(--edinsa-blue);font-weight:900}.ops>header span{color:#667b84}.grid{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(0,.9fr);gap:1rem}.panel{background:#fff;border:1px solid #d9e4e8;border-radius:14px;padding:1rem}.panel label{display:grid;gap:.3rem;margin-bottom:.8rem;font-size:.72rem;font-weight:700;color:#526a75}select,input,textarea{max-width:100%;min-width:0;padding:.65rem;border:1px solid #cbd9de;border-radius:8px;background:#fff}.selection{background:#eaf5f8;padding:.75rem;border-radius:8px}.warning{padding:.8rem;background:#fff1d9;border-left:4px solid #ff962e}.primary{width:100%;padding:.75rem;border:0;border-radius:9px;color:#fff;font-weight:800;background:linear-gradient(110deg,var(--edinsa-blue),var(--edinsa-green))}.message{padding:.7rem;background:#eef7e9}@media(max-width:850px){.grid{grid-template-columns:1fr}}`]} )
export class MovementsPage implements OnInit {
 ...
 select(p:DiagramPosition){
  ...
  this.selectedPosition.set(p);if(this.type!=='Reemplazar llanta')this.tireId=raw?.llantaId??this.tireId;
 }
 ...
 async submit(){
  ...
  if(this.type==='Reemplazar llanta'){
   const current=this.positions().find(p=>p.id===source?.id)?.llantaId;
   if(!source||!current||current===this.tireId)return this.message.set('Selecciona una posición ocupada y una llanta nueva diferente.');
   this.assignments=[{posicionId:String(source.id),llantaId:this.tireId,llantaActualId:current}];
   return this.submitSet('Reemplazar llanta');
  }
  if(this.type==='Rotación'&&(!this.destinationPositionId||this.destinationPositionId===source?.id))return this.message.set('Selecciona otra posición del mismo vehículo.');
 ...
 async submitSet(tipo='Cambio de juego'){if(this.busy())return;if(tipo==='Cambio de juego'&&this.assignments.length<2)return this.message.set('Cambio de juego requiere al menos dos posiciones. Usa Reemplazar llanta para una sola.');if(!this.assignments.length||!this.reason.trim())return this.message.set('Asigna llantas e ingresa el motivo.');if(!this.mileageValid())return;this.busy.set(true);try{await firstValueFrom(this.http.post('/api/operaciones/solicitudes',{tipo,vehiculoId:this.vehicleId,asignaciones:this.assignments,motivo:this.reason,observaciones:this.notes,kilometrajeVehiculo:this.mileage}));this.message.set(tipo+' enviado para autorización.');this.assignments=[];await this.refresh();}catch(e:any){this.message.set(e?.userMessage??'No fue posible enviar el cambio de juego.');}finally{this.busy.set(false);}}
