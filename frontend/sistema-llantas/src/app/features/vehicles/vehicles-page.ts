...
import {VehicleConfiguration,VehicleConfigurationInput,VehicleDetail,VehicleInput,VehicleSummary,VehiclesApi} from './vehicles-api';

@Component({selector:'app-vehicles-page',imports:[FormsModule,VehicleAxleDiagram,DatePipe],templateUrl:'./vehicles-page.html',styleUrls:['./vehicles-page.scss','./configuration-builder.scss','./configuration-catalog.scss']})
export class VehiclesPage implements OnInit{
 private readonly api=inject(VehiclesApi);private readonly catalogs=inject(CatalogsApi);readonly auth=inject(AuthService);
 ...
 search='';showForm=false;showConfiguration=false;editing=false;form:VehicleInput=this.blank();configurationForm:VehicleConfigurationInput=this.blankConfiguration();
 readonly diagram=computed<DiagramAxle[]>(()=>this.selected()?.ejes.map(e=>({id:e.id,name:e.nombre,type:e.tipoEje,positions:e.posiciones.map(p=>({id:p.id,code:p.codigo,side:[p.lado,p.ubicacion].filter(Boolean).join(' · '),tire:p.llantaCodigo??undefined,state:p.llantaCodigo?'normal':'empty'}))}))??[]);
 ...
 async open(id:string){try{this.selected.set(await firstValueFrom(this.api.get(id)))}catch{this.message.set('No fue posible cargar el detalle del vehículo.')}}
